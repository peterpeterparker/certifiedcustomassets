...
fn build_certified_headers_impl(Asset { key, headers: _, encoding: _ }: &Asset, state: &RuntimeState) -> Result<HeaderField, &'static str> {
    build_asset_certificate_header(&state.asset_hashes, key.fullPath.clone())
}
